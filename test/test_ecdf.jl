...
@testset "empirical cdf" begin
    ...
    @testset "calculate_bin_bounds" begin
        @testset "brackets the data" begin
            data = collect(1.0:100.0)
            lo, hi = calculate_bin_bounds(data)
            @test lo < hi
        ...
        @testset "constant data collapses to a hairline range" begin
            ...
            # in calculate_bin_bounds never fires on constant data.
            lo, hi = calculate_bin_bounds(fill(5.0, 20))
            @test hi > lo
            @test hi - lo ≈ 0.5e-9 rtol = 1e-6
        end

        @testset "symmetric data gives symmetric bounds" begin
            lo, hi = calculate_bin_bounds(collect(-10.0:10.0))
            @test lo ≈ -hi
        end
        ...
        @testset "shifting the data shifts the bounds" begin
            ...
            lo, hi = calculate_bin_bounds(data)
            lo2, hi2 = calculate_bin_bounds(data .+ 100.0)
            @test lo2 ≈ lo + 100.0
            @test hi2 ≈ hi + 100.0
        end
    end

    @testset "bin_select" begin
        data = collect(1.0:100.0)

        @testset ":xax is uniformly spaced" begin
            bins = bin_select(data, 8, :xax)
            @test length(bins) == 8
            @test issorted(bins)
        ...
        @testset ":log is positive and geometrically spaced" begin
            ...
            bins = bin_select(collect(1000.0:1100.0), 6, :log)
            @test length(bins) == 6
            @test all(>(0), bins)
            @test issorted(bins)
    ...
    @testset "initialize_bins dispatch" begin
        data = row(1.0:40.0)
        ...
        @testset "StandardECDFSummary gets one bin vector per data row" begin
            stat = initialize_bins(dc, StandardECDF(5), opts, cache)
        ...
        @testset "multidimensional data gets one bin vector per row" begin
            ...
            stat = initialize_bins(dc2, StandardECDF(5, 2), dc2.options, cache)
        ...
        @testset "non-ECDF summaries pass through untouched" begin
            for stat in (ChamferDistance(1), ChamferDistanceDiff(1, 1, 1.0), CumulativeSum(4))
                @test initialize_bins(dc, stat, opts, cache) === stat
            end
        end
    end
end
